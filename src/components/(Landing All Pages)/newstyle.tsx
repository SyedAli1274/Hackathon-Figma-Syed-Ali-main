...
export default function Newstyle() {
  const [products, setProducts] = useState<Products[]>([]); // To store fetched products
  const [loading, setLoading] = useState<boolean>(true); // Track loading state
  const [error, setError] = useState<string | null>(null); // Track error state
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const productsData = await client.fetch(`
          *[_type == "products" && slug.current in ["citrus-edge","ivory-charm","desk-chair" ,"nordic-spin" ,"scandi-dip-set"]]{
      title,
      price,
      stock,
      slug,
