...
export default function Navbar() {
  ...
  // States for search
  const [searchTerm, setSearchTerm] = useState<string>(""); 
  const [allProducts, setAllProducts] = useState<Products[]>([]); // All products should be an array of Product
  const [filteredProducts, setFilteredProducts] = useState<Products[]>([]); // Filtered products should be an array of Product
              ...
              {filteredProducts.length > 0 ? (
                <ul className="space-y-2">
                  {filteredProducts.map((products) => (
                    <li key={products._id}>
                      <Link href={`/products/${products.slug.current}`} className="text-blue-600 hover:underline">
                        {products.title}
                      </Link>
                    </li>
                  ))}
